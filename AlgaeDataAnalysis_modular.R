...
#### Plotting corrected batch effects
...
for(i in 1:15)#ncol(ms_data_day4_nonzero))
{
  ...
  points(sigfeat_day4_bc_rt_r,sigfeat_day4_bc_mz_r,pch=1,col="green")
  ...
  plot(as.numeric(day4_nonzero_fvalues_s[,i]),as.numeric(day4_nonzero_r2F_s[,i]),pch=1,main="day 4 Strain-F val VS r2 statistic",ylab="r2",xlab="fvalue",col=ifelse(sigfeat_day4_s=="sig","red","#00000033"))
  legend(x="topright",inset=0, legend=c("sig","non-sig"),col=c("red","black"),pch=1)  
  
  plot(log(df2_r$newBin),df2_r$nosig,type='l', main="RunDay-test statistic variance Vs Sig features",ylim=c(0,max(df2_r$nosig,df2_r$sig)))
  ...
  legend(x="top",inset=0, legend=c(paste0("PC's removed=", paste(unlist(pc_combi_list_day4[i]),collapse=",")),
                                   paste0("Residual variance remaining=",resid_var)), horiz=TRUE, cex=1)
...
#day12
for(i in 1:4)#ncol(ms_data_day12_nonzero))
  {
    ...
    sigfeat_day12_bc_mz_r<-sapply(sigfeat_day12_bc_r , function (x) if(length(x) == 2) x[1] else as.character(NA))
    sigfeat_day12_bc_rt_r<-sapply(sigfeat_day12_bc_r , function (x) if(length(x) == 2) x[2] else as.character(NA))
    tot_mz_r<-length(sigfeat_day12_bc_mz_r)
    ...
    tot_mz_s<-length(sigfeat_day12_bc_mz_s)
    
    resid_var<-round(100-sum(residual_variance_day12_mzbysam_princomp[1:i])*100,2)
    day12_numdenum_r<-day12_nonzero_denum_values_r[,i]/day12_nonzero_num_values_r[,i]
    day12_numdenum_r_ind<-match(sigfeat_day12_bc_r,mz_rt_day12); day12_numdenum_r_ind<-day12_numdenum_r_ind[!is.na(day12_numdenum_r_ind)] #obtaining indices only for the significant features
    day12_numdenum_s<-day12_nonzero_denum_values_s[,i]/day12_nonzero_num_values_s[,i]
    ...
    tmp1<-day12_numdenum_r[day12_numdenum_r_ind]
    tmp1<-tmp1[tmp1<0.1]
    tmp2<-day12_numdenum_s[day12_numdenum_s_ind]
    tmp2<-tmp2[tmp2<0.1]
    
    #plotting test statistic vs no-of-sig-features runday
    sigfeat_day12_r<-rep('nosig',length(rt_day12))
    sigfeat_day12_r[day12_numdenum_r_ind]<-'sig'
    
    df1_r<-data.frame(as.numeric(day12_nonzero_denum_values_r[,i]),sigfeat_day12_r)
    colnames(df1_r)<-c('denom','sigfeat')
    df1_r$bins <- cut2(df1_r$denom, c(0.025,0.05,0.1,0.5,1,1.5,2,3))
    df2_r<-as.data.frame.matrix(table(df1_r$bins,df1_r$sigfeat))
    #colnames(df2_r)<-c('bin','nosig','sig')
    ...
    points(sigfeat_day12_bc_rt_r,sigfeat_day12_bc_mz_r,pch=1,col="green")
    ...
    plot(as.numeric(day12_nonzero_fvalues_s[,i]),as.numeric(day12_nonzero_r2F_s[,i]),pch=1,main="day 12 Strain-F val VS r2 statistic",ylab="r2",xlab="fvalue",col=ifelse(sigfeat_day12_s=="sig","red","#00000033"))
    legend(x="topright",inset=0, legend=c("sig","non-sig"),col=c("red","black"),pch=1)  
    
    plot(log(df2_r$newBin),df2_r$nosig,type='l', main="RunDay-test statistic variance Vs Sig features",ylim=c(0,max(df2_r$nosig,df2_r$sig)))
    lines(log(df2_r$newBin),df2_r$sig,col="red")
    ...
    plot(1,type="n", axes=FALSE, xlab="", ylab="")
    legend(x="top",inset=0, legend=c(paste0("PC's removed=", paste(unlist(pc_combi_list_day12[i]),collapse=",")),
                                     paste0("Residual variance remaining=",resid_var)), horiz=TRUE, cex=1)
