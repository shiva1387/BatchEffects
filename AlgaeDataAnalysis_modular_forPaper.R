...
# Functions written here are used in shiv.mv.functions.R and AlgaeDataAnalysis_server_v1.R
...
pdf("biochem_pairsPlot.pdf",height=16,width=16)
pairs(biochem_para[,3:11],lower.panel=panel.smooth)
dev.off()
...
###functions associated with mt.maxT
# compute permutative f test statistics to identify signigicant features
...
compute_perm_ftest_singleDataFrame<-function(dataset,classlabel) {
    classlabel_factor<-as.numeric(as.factor(classlabel))-1
...
compute_no_features<-function(sig_feat) {
  ...
  {
    no_of_sig_features[i]<-nrow(sig_feat[[i]]) #updated by shiv 03-feb-2014
  }
  return(no_of_sig_features)
}

### functions associated with num.denum
# compute numerator and denominator of test statistic using mt.teststat.num.denum from mulltest
compute_numdenum_ftest<-function(dataset,classlabel) {
  classlabel_factor<-as.numeric(as.factor(classlabel))-1
  if(length(dataset)> 1 ) {
    numdenum_metab_dataset<-vector("list", length(dataset)) 
    for(i in 1:length(dataset))
    { data_matrix<-as.matrix(dataset[[i]])
    ...
    classlabel_factor<-as.numeric(as.factor(classlabel))-1
    numdenum_metab_dataset<-list()
    data_matrix<-as.matrix(dataset[[1]])
    dataset_numdenum<-mt.teststat.num.denum(data_matrix,classlabel_factor,test="f",nonpara="n")
    numdenum_metab_dataset[[1]]<-dataset_numdenum
  }
  
  #return(numdenum_metab_dataset) # return list of df(num column and denum column)-for each svd combination
  #extract num denum for each dataframe from list 
  # the below function splits each df containing num and denum column
  # into num matrix  and denum matrix for all svd combinations
  # this function can be comnined with the compute_numdenum_ftest
  
  listofnum<-vector("list", length(numdenum_metab_dataset)) #list()
  listofdenum<-listofnum
  for(i in 1:length(numdenum_metab_dataset))
  {
    listofnum[i]<-numdenum_metab_dataset[[i]][1]
    listofdenum[i]<-numdenum_metab_dataset[[i]][2]
  }
  num_teststat<-do.call(cbind.data.frame, listofnum)
  denum_teststat<-do.call(cbind.data.frame, listofdenum)
  return (list(num_teststat,denum_teststat))
}
...
compute_r2_from_fvalue<-function(dataset,classlabel) {
  classlabel_factor<-as.numeric(as.factor(classlabel))-1
  if(length(dataset)> 1 ) {
    fval_teststat<-as.data.frame(NA)
    r2_teststat<-as.data.frame(NA)
    for(i in 1:length(dataset))
    { data_matrix<-as.matrix(dataset[[i]])
      ...
      fval_teststat<-cbind(fval_teststat,dataset_fvalue)
      r2_teststat<-cbind(r2_teststat,dataset_r2)
    }
  } 
  else { # only a single dataset
    classlabel_factor<-as.numeric(as.factor(classlabel))-1
    fvalue_metab_dataset<-vector("list", length(dataset)) 
    fval_teststat<-as.data.frame(matrix(NA,nrow(svd_day12_nonzero[[1]]),1))
    r2_teststat<-as.data.frame(matrix(NA,nrow(svd_day12_nonzero[[1]]),1))
    data_matrix<-as.matrix(dataset)
    dataset_fvalue<-mt.teststat(data_matrix,classlabel_factor,test="f",nonpara="n")
    dataset_r2<-r2.from.Fstat(dataset_fvalue,length(unique(classlabel)),ncol(data_matrix))
  ...
  fval_teststat<-fval_teststat[,2:ncol(fval_teststat)]
  r2_teststat<-r2_teststat[,2:ncol(r2_teststat)]
...
start_pc_comp=1;end_pc=ncol(ms_data_day4_nonzero) #the number used for calculating the last pc combination
pc_combi_list_day4<-sapply(start_pc_comp:end_pc,function(x) seq(start_pc_comp:x)) #function used to provide pc combinations.For ex 1,1:2,1:2:3 here if start_pc_comp=1 and end_pc=3
...
day4_nonzero_denum_values_r<-day4_nonzero_numdenum_r[[2]]
...
day4_nonzero_denum_values_s<-day4_nonzero_numdenum_s[[2]]
...
#######################################################
## Compute diff featues, r2 and overlaps for uncorrected dataset
########################################################
day4_uncorrected_nonzero_sigfeat_r<-compute_perm_ftest_singleDataFrame(ScaleData(ms_data_day4_nonzero),RunDay_day4)
day4_uncorrected_nonzero_sigfeat_r_pval<-day4_uncorrected_nonzero_sigfeat_r[[1]]
day4_uncorrected_nonzero_sigfeat_r_pval$BHadj<-p.adjust(as.numeric(day4_uncorrected_nonzero_sigfeat_r_pval$rawp),method="BH")
day4_uncorrected_nonzero_sigfeat_r_matrix<-day4_uncorrected_nonzero_sigfeat_r[[2]]
# a<-day4_uncorrected_nonzero_sigfeat_r_pval$BHadj;b<-a[a<0.05];c<-p.adjust(b,method="BH");d<-c[c<0.05];length(d) #10,537
day4_uncorrected_nonzero_sigfeat_r_index<-sort(day4_uncorrected_nonzero_sigfeat_r_pval[day4_uncorrected_nonzero_sigfeat_r_pval$BHadj < 0.05,c(1)])

day4_uncorrected_nonzero_fval_r2_r<-compute_r2_from_fvalue_singleDataFrame(ScaleData(ms_data_day4_nonzero),RunDay_day4)
...
day4_uncorrected_nonzero_sigfeat_s_pval$BHadj<-p.adjust(as.numeric(day4_uncorrected_nonzero_sigfeat_s_pval$rawp),method="BH")
day4_uncorrected_nonzero_sigfeat_s_matrix<-day4_uncorrected_nonzero_sigfeat_s[[2]]
day4_uncorrected_nonzero_sigfeat_s_index<-sort(day4_uncorrected_nonzero_sigfeat_s_pval[day4_uncorrected_nonzero_sigfeat_s_pval$BHadj < 0.05,c(1)])
# length(day4_uncorrected_nonzero_sigfeat_s_index) #13012
day4_uncorrected_nonzero_fval_r2_s<-compute_r2_from_fvalue_singleDataFrame(ScaleData(ms_data_day4_nonzero),RunDay_day4)
day4_uncorrected_nonzero_fvalues_s<-day4_uncorrected_nonzero_fval_r2_s[[1]]
...
start_pc_comp=1;end_pc=ncol(ms_data_day12_nonzero) #the number used for calculating the last pc combination
pc_combi_list_day12<-sapply(start_pc_comp:end_pc,function(x) seq(start_pc_comp:x)) #function used to provide pc combinations.For ex 1,1:2,1:2:3 here if start_pc_comp=1 and end_pc=3
...
day12_nonzero_denum_values_s<-day12_nonzero_numdenum_s[[2]]
...
## Compute diff featues, r2 and overlaps for uncorrected dataset
########################################################
day12_uncorrected_nonzero_sigfeat_r<-compute_perm_ftest_singleDataFrame(ScaleData(ms_data_day12_nonzero),RunDay_day12)
day12_uncorrected_nonzero_sigfeat_r_pval<-day12_uncorrected_nonzero_sigfeat_r[[1]]
day12_uncorrected_nonzero_sigfeat_r_pval$BHadj<-p.adjust(as.numeric(day12_uncorrected_nonzero_sigfeat_r_pval$rawp),method="BH")
day12_uncorrected_nonzero_sigfeat_r_matrix<-day12_uncorrected_nonzero_sigfeat_r[[2]]
# a<-day12_uncorrected_nonzero_sigfeat_r_pval$BHadj;b<-a[a<0.05];c<-p.adjust(b,method="BH");d<-c[c<0.05];length(d) #8,910
day12_uncorrected_nonzero_sigfeat_r_index<-sort(day12_uncorrected_nonzero_sigfeat_r_pval[day12_uncorrected_nonzero_sigfeat_r_pval$BHadj < 0.05,c(1)])

day12_uncorrected_nonzero_fval_r2_r<-compute_r2_from_fvalue_singleDataFrame(ScaleData(ms_data_day12_nonzero),RunDay_day12)
...
day12_uncorrected_nonzero_sigfeat_s_pval<-day12_uncorrected_nonzero_sigfeat_s[[1]]
day12_uncorrected_nonzero_sigfeat_s_pval$BHadj<-p.adjust(as.numeric(day12_uncorrected_nonzero_sigfeat_s_pval$rawp),method="BH")
day12_uncorrected_nonzero_sigfeat_s_matrix<-day12_uncorrected_nonzero_sigfeat_s[[2]]
day12_uncorrected_nonzero_sigfeat_s_index<-sort(day12_uncorrected_nonzero_sigfeat_s_pval[day12_uncorrected_nonzero_sigfeat_s_pval$BHadj < 0.05,c(1)])
# length(day12_uncorrected_nonzero_sigfeat_s_index) #10309

day12_uncorrected_nonzero_fval_r2_s<-compute_r2_from_fvalue_singleDataFrame(ScaleData(ms_data_day12_nonzero),RunDay_day12)
day12_uncorrected_nonzero_fvalues_s<-day12_uncorrected_nonzero_fval_r2_s[[1]]
...
## Figure 3
# Obtain averaged r2 for differatial features in each pc belonging to strain 
### Day 4
no_sigFeat_d4_r<-rep(0,ncol(ms_data_day4_nonzero))
no_sigFeat_d4_s<-rep(0,ncol(ms_data_day4_nonzero))
r2_d4_r_significant<-rep(0,ncol(ms_data_day4_nonzero))
r2_d4_s_significant<-rep(0,ncol(ms_data_day4_nonzero))
sigfeat_overlap_d4<-rep(0,ncol(ms_data_day4_nonzero))
resid_var_d4<-rep(0,ncol(ms_data_day4_nonzero))

for(i in 1:ncol(ms_data_day4_nonzero))
{
  ...
  sigfeat_r_ind_d4<-0
  sigfeat_s_ind_d4<-0
  if(nrow(day4_nonzero_sigfeat_r_matrix[[i]])>0)
    {
        sigfeat_day4_bc_r<-strsplit(rownames(day4_nonzero_sigfeat_r_matrix[[i]]), "\\@")
        #obtaing the index for significant features associated with strain and run day
        sigfeat_r_ind_d4<-match(sigfeat_day4_bc_r,mz_rt_day4); sigfeat_r_ind_d4<-sigfeat_r_ind_d4[!is.na(sigfeat_r_ind_d4)] #runday
        # this is done as r2 is calculated for all the features. There we want to find the average r2 for all features associated 
        # with (1) strain and (2) run day
        no_sigFeat_d4_r[i]<-length(sigfeat_r_ind_d4)
        r2_d4_r<-as.numeric(day4_nonzero_r2F_r[,i])
        r2_d4_r_significant0<-r2_d4_r[sigfeat_r_ind_d4]
        r2_d4_r_significant[i]<-round(sum(r2_d4_r_significant0)/length(r2_d4_r_significant0),2)
    }
  if(nrow(day4_nonzero_sigfeat_s_matrix[[i]])>0) 
  {
    sigfeat_day4_bc_s<-strsplit(rownames(day4_nonzero_sigfeat_s_matrix[[i]]), "\\@")
    
    sigfeat_s_ind_d4<-match(sigfeat_day4_bc_s,mz_rt_day4); sigfeat_s_ind_d4<-sigfeat_s_ind_d4[!is.na(sigfeat_s_ind_d4)] #strain
    no_sigFeat_d4_s[i]<-length(sigfeat_s_ind_d4)
    r2_d4_s<-as.numeric(day4_nonzero_r2F_s[,i])
    r2_d4_s_significant0<-r2_d4_s[sigfeat_s_ind_d4]
    r2_d4_s_significant[i]<-round(sum(r2_d4_s_significant0)/length(r2_d4_s_significant0),2)
  }
  sigfeat_overlap_d4[i]<-round((length(match(sigfeat_day4_bc_r,sigfeat_day4_bc_s))/nrow(ms_data_day4_nonzero))*100,2)
  
  resid_var_d4[i]<-round(100-sum(residual_variance_day4_mzbysam_princomp[1:i])*100,2)
  
} # end of for loop

### Plotting ..melting dataset for plots

no_sigFeat_d4_r[is.na(no_sigFeat_d4_r)]<-0
r2_d4_r_significant[is.na(r2_d4_r_significant)]<-0
no_sigFeat_d4_s[is.na(no_sigFeat_d4_s)]<-0
...
no_sigFeat_d12_s<-rep(0,ncol(ms_data_day12_nonzero))
r2_d12_r_significant<-rep(0,ncol(ms_data_day12_nonzero))
r2_d12_s_significant<-rep(0,ncol(ms_data_day12_nonzero))
sigfeat_overlap_d12<-rep(0,ncol(ms_data_day12_nonzero))
resid_var_d12<-rep(0,ncol(ms_data_day12_nonzero))

for(i in 1:ncol(ms_data_day12_nonzero))
{
  ...
  sigfeat_r_ind_d12<-0
  sigfeat_s_ind_d12<-0
  if(nrow(day12_nonzero_sigfeat_r_matrix[[i]])>0)
  {
    sigfeat_day12_bc_r<-strsplit(rownames(day12_nonzero_sigfeat_r_matrix[[i]]), "\\@")
    #obtaing the index for significant features associated with strain and run day
    sigfeat_r_ind_d12<-match(sigfeat_day12_bc_r,mz_rt_day12); sigfeat_r_ind_d12<-sigfeat_r_ind_d12[!is.na(sigfeat_r_ind_d12)] #runday
    # this is done as r2 is calculated for all the features. There we want to find the average r2 for all features associated 
    # with (1) strain and (2) run day
    no_sigFeat_d12_r[i]<-length(sigfeat_r_ind_d12)
    r2_d12_r<-as.numeric(day12_nonzero_r2F_r[,i])
    r2_d12_r_significant0<-r2_d12_r[sigfeat_r_ind_d12]
    r2_d12_r_significant[i]<-round(sum(r2_d12_r_significant0)/length(r2_d12_r_significant0),2)
  }
  if(nrow(day12_nonzero_sigfeat_s_matrix[[i]])>0) 
  {
    sigfeat_day12_bc_s<-strsplit(rownames(day12_nonzero_sigfeat_s_matrix[[i]]), "\\@")
    
    sigfeat_s_ind_d12<-match(sigfeat_day12_bc_s,mz_rt_day12); sigfeat_s_ind_d12<-sigfeat_s_ind_d12[!is.na(sigfeat_s_ind_d12)] #strain
    no_sigFeat_d12_s[i]<-length(sigfeat_s_ind_d12)
    r2_d12_s<-as.numeric(day12_nonzero_r2F_s[,i])
    r2_d12_s_significant0<-r2_d12_s[sigfeat_s_ind_d12]
    r2_d12_s_significant[i]<-round(sum(r2_d12_s_significant0)/length(r2_d12_s_significant0),2)
  }
  sigfeat_overlap_d12[i]<-round((length(match(sigfeat_day12_bc_r,sigfeat_day12_bc_s))/nrow(ms_data_day12_nonzero))*100,2)
  
  resid_var_d12[i]<-round(100-sum(residual_variance_day12_mzbysam_princomp[1:i])*100,2)
  
} # end of for loop

### Plotting ..melting dataset for plots

no_sigFeat_d12_r[is.na(no_sigFeat_d12_r)]<-0
r2_d12_r_significant[is.na(r2_d12_r_significant)]<-0
no_sigFeat_d12_s[is.na(no_sigFeat_d12_s)]<-0
...
# biochem_para$Protein.percen       1  0.004657 0.0046570 10.1994 0.05598  0.001 ***
#   biochem_para$Carb.percent         1  0.005325 0.0053247 11.6617 0.06400  0.001 ***
#   biochem_para$lipid.produc         1  0.010129 0.0101288 22.1832 0.12175  0.001 ***
#   biochem_para$max.biomass.density  1  0.008867 0.0088668 19.4194 0.10658  0.001 ***
#   biochem_para$avg.OD               1  0.008892 0.0088920 19.4746 0.10688  0.001 ***
#   biochem_para$RunDay               1  0.008218 0.0082178 17.9980 0.09878  0.001 ***
#   biochem_para$Strain               4  0.015649 0.0039122  8.5681 0.18809  0.001 ***
#   Residuals                        47  0.021460 0.0004566         0.25795           
# Total                            57  0.083196                   1.00000           
# ---
...
# 
# Call:
#   adonis(formula = t(log(permutedData)) ~ biochem_para$Protein.percen +      biochem_para$Carb.percent + biochem_para$lipid.produc + biochem_para$max.biomass.density +      biochem_para$avg.OD + biochem_para$RunDay + biochem_para$Strain,      permutations = 999, method = "bray") 
# 
# Terms added sequentially (first to last)
# 
# Df SumsOfSqs    MeanSqs F.Model      R2 Pr(>F)  
# biochem_para$Protein.percen       1  0.001005 0.00100513 0.71990 0.01208  0.710  
# biochem_para$Carb.percent         1  0.002262 0.00226245 1.62042 0.02719  0.100 .
# biochem_para$lipid.produc         1  0.001282 0.00128198 0.91819 0.01541  0.503  
# biochem_para$max.biomass.density  1  0.000881 0.00088089 0.63091 0.01059  0.817  
# biochem_para$avg.OD               1  0.001095 0.00109481 0.78413 0.01316  0.623  
# biochem_para$RunDay               1  0.002287 0.00228724 1.63818 0.02749  0.089 .
# biochem_para$Strain               4  0.008761 0.00219037 1.56880 0.10531  0.023 *
#   Residuals                        47  0.065622 0.00139621         0.78876         
# Total                            57  0.083196                    1.00000         
# ---
#   Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
# 
# Df SumsOfSqs  MeanSqs F.Model       R2 Pr(>F)
# biochem_para$Protein.percen       1   1404591  1404591  346.64  0.33619  0.203
# biochem_para$Carb.percent         1   -660600  -660600 -163.03 -0.15811  0.819
# biochem_para$lipid.produc         1  -3830044 -3830044 -945.22 -0.91672  0.951
# biochem_para$max.biomass.density  1   3561062  3561062  878.83  0.85234  0.119
# biochem_para$avg.OD               1    862067   862067  212.75  0.20633  0.222
# biochem_para$RunDay               1   2835872  2835872  699.86  0.67876  0.144
# biochem_para$Strain               4   -185396   -46349  -11.44 -0.04437  0.793
# Residuals                        47    190445     4052          0.04558       
# Total                            57   4177998                   1.00000 
